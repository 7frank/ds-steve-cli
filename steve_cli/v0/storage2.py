...
class S3Storage:
    ...
    def _handle_client_error(
        ...
        if code == "AccessDenied":
            print(
                f"❌ Access denied when {operation}: {path}",
                file=sys.stderr,
            )
    ...
    def put_file(self, local_path: str, path: str) -> None:
        try:
            ...
            print(
                f"❌ Cannot connect to storage at {self.endpoint}",
                file=sys.stderr,
            )
            sys.exit(1)
...
# TODO env vars for traces passed by etl pipeline

# OPENLINEAGE_RUN_ID=6b3c8e4f-...
# OPENLINEAGE_NAMESPACE=analytics
# OPENLINEAGE_JOB_NAME=customer_pipeline
...
def get_storage(
    ...
    if lineage_enabled:
        ...
        new_var = OpenLineageCollector(
            namespace=workspace or "default",
